...
const BGM = {
  ...
  noteFreq: function(note) {
    ...
    const N = {C:0,Cs:1,Db:1,D:2,Ds:3,Eb:3,E:4,F:5,Fs:6,Gb:6,G:7,Gs:8,Ab:8,A:9,As:10,Bb:10,B:11};
    const m = String(note).match(/^([A-G][sb]?)(\d)$/);
    if (!m) return 0;
    const n = N[m[1]]; if (n === undefined) return 0;
  ...
  play: function(trackName, fadeIn) {
    ...
    this._hardStop();
    const track = BGM.tracks[trackName];
  ...
  stop: function(fadeOut) {
    if (fadeOut && this.ctx && this.masterGain) {
      ...
      const s = this; setTimeout(function() { s._hardStop(); }, 550);
    } else { this._hardStop(); }
  },
  ...
  _schedule: function() {
    ...
    const sps = 60.0 / this.tempo / this.stepsPerBeat;
    while (this.nextNoteTime < this.ctx.currentTime + 0.1) {
      this._playStep(this.currentStep, this.nextNoteTime, sps);
      this.nextNoteTime += sps;
      this.currentStep++;
      const total = this.currentTrack.patternLength * this.stepsPerBeat * 4;
      if (this.currentStep >= total) {
        if (this.currentTrack.loop === false) { this._hardStop(); return; }
        this.currentStep = 0;
      }
    }
    const s = this; this.schedulerTimer = setTimeout(function() { s._schedule(); }, 25);
  },

  _playStep: function(step, time, dur) {
    for (let i = 0; i < this.channels.length; i++) {
      const d = this.channels[i].def, p = d.pattern;
      const note = p[step % p.length];
      if (!note || note === '-' || note === 0) continue;
      const freq = (typeof note === 'number' && note > 20) ? note : this.noteFreq(note);
      if (!freq || freq < 20) { if (typeof note === 'number') { this._noise(time, dur*(d.sustain||0.8), d.volume||0.5); } continue; }
      const vol = d.volume || 0.5, type = d.type || 'square', nd = dur * (d.sustain || 0.8);
  ...
  _noise: function(time, dur, vol) {
    ...
    const buf = this.ctx.createBuffer(1, len, sr), data = buf.getChannelData(0);
  ...
  setVolume: function(v) {
    ...
    const el = document.getElementById('bgm-vol-display');
  ...
  toggleMute: function() {
    ...
    const el = document.getElementById('btn-mute-inner');
    if (el) el.textContent = this.muted ? '🔇' : '🔊';
    return this.muted;
  },

  createVolumeControl: function() {
    const c = document.createElement('div'); c.id = 'bgm-controls';
    ...
    const topBar = document.getElementById('top-bar'); if (topBar) { topBar.appendChild(c); } else { document.body.appendChild(c); }
    const s = this;
